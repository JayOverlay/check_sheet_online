...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    try {
        ...
        // 5. If NG found → Redirect to Downtime page to report issue
        if ($has_problem) {
            // Build problem description from NG items for pre-fill
            $problem_desc = implode(" | ", $problems_found);
            if (!empty($overall)) {
                $problem_desc .= " [Overall: $overall]";
            }

            // Redirect to downtime page with report modal auto-open
            ...
            header("Location: " . BASE_URL . "pages/downtime.php?" . $redirect_params);
        } else {
            // 6. Redirect to Success Page (all OK)
